...
template <typename data_structure, typename data_type>
bool hash< void( data_structure, data_type ) >::remove(char * key)
{
	return remove( key, & hash_table[ hash_function( key ) ] );
}



template <typename data_structure, typename data_type>
bool hash< void( data_structure, data_type ) >::remove(char * key, data_structure ** curr)
{
	if ( *curr )
	{
